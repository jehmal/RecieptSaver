...
const ACTION_BUTTON_WIDTH = 75; // Reduced from 80 to fit better
const MAX_SWIPE_DISTANCE = screenWidth * 0.4; // Maximum 40% of screen width

interface SwipeAction {
  type: 'categorize' | 'archive' | 'delete';
  color: string;
  icon: string;
  label: string;
  onPress: () => void;
...
export const SwipeableReceiptCard: React.FC<SwipeableReceiptCardProps> = ({
  ...
  const handleSwipeableWillOpen = (direction: 'left' | 'right') => {
    // Provide visual feedback before action
    triggerHaptic('light');
  };
  ...
  // When swipe is disabled, render children directly to allow inner gesture handlers to work
...
export default SwipeableReceiptCard;